...
while ~push_button;
    ...
    Screen('TextSize',scr.main, const.instsize);
    ...
    espace = ((const.instsize)*1.50);
    first_line = scr.y_mid - ((round(lines/2))*espace);

    addi = 0;
    for t_lines = 1:sizeT(1)
        Screen('DrawText',scr.main,text{t_lines,:},scr.x_mid-bound(3)/2,first_line+addi*espace, const.colorT);
        addi = addi+1;
    end
    addi = addi+2;
    for b_lines = 1:sizeB(1)
        Screen('DrawText',scr.main,button{b_lines,:},scr.x_mid-bound(3)/2,first_line+addi*espace, const.colorT);
    end
